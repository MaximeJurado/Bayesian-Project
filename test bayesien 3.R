...
mod2.5 <- bugs(dataB13, inits=inits13, parameters.to.save=para13,
               ...
               bugs.directory=path.WBS,               
               n.iter=(Iter*Thin+Burn),n.burnin=Burn, n.thin=Thin, n.chains=Chain, DIC=T, debug=T)
print(mod2.5, digits=4) #spc² random effect spc²
...
sum(res2.5^2,na.rm=TRUE)
res2.5<-subset(res2.5,!is.na(res2.5))

hist(res2.5, freq=FALSE)
lines(density(res2.5, adjust = 1), col=2)
pred2.5.cuts <- cut2(pred2.5, cuts=c(quantile(pred2.5, c(0,0.2,0.4,0.6,0.8,1), na.rm=TRUE)))
levels(pred2.5.cuts) <- c("1","2","3","4","5")
sum(ifelse(rent.cuts == pred2.5.cuts, 1, 0), na.rm=TRUE)
detach.bugs()
par(mfrow=c(1,1),mar=c(3,3,3,3))
plot(1:73, pred1.4, pch=16, xlab="", ylab="", main="Mean Posterior Predictive", ylim=c(600,2000))
